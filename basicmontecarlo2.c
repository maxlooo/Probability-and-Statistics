...
double random(void); 
void allocateSample(int sampleSize, double sample[POPSIZE][sampleSize]);
void distributeSamples(int sampleSize, double sample[POPSIZE][sampleSize], int x, int value[][20]);

int main(void) {
  const int sampleSize5=5;
  const int sampleSize10=10;
  int value[2][20]={};
  double sample1[POPSIZE][sampleSize5], sample2[POPSIZE][sampleSize10];
  srand(time(0)); 
  ...
  distributeSamples(sampleSize5, sample1, 0, value);
  // distribute average for each sample size 10
  distributeSamples(sampleSize10, sample2, 1, value);

  // print relative frequency histogram
  printf("\n*****Relative Frequency Histogram of Average of Sample Size Five 1000 times*****\n");
  ...
  // print relative frequency histogram
  printf("\n*****Relative Frequency Histogram of Average of Sample Size Ten 1000 times*****\n");
  printf("From 0 to 1.0, segmented into 20 parts\n");
  printf("\n");
  for (int i=0; i<20; ++i) {
    printf("%d, ", value[1][i]);
...
// generate samples 1000 times from random uniform distribution
void allocateSample(int sampleSize, double sample[POPSIZE][sampleSize]) {
  int count1, count2;
